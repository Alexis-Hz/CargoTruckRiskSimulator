...
hasThreat(V) :- hasAnomaly(V,A), hasNoJustification(V,A).
...
hasAnomaly(V,A) :- hasMonitoringAnomaly(V,A);
                   hasTimeDeviation(V,A);
...
hasMonitoringAnomaly(V,A) :- isNotMonitored(V), A='M';
                             hasNoAssignedRoute(V,_), A='M';
                             hasNoActualRoute(V,_), A='M';
                             hasNoExpectedArrivalTime(V,_), A='M';
                             hasNoActualArrivalTime(V,_), A='M'.
...
hasTimeDeviation(V,D) :- hasExpectedArrivalTime(V,Te), hasActualArrivalTime(V,Ta), notOnTime(Te,Ta),D='T'.
...
hasRouteDeviation(V,D) :- hasAssignedRoute(V,Ra), hasActualRoute(V,Rr), areNotEqualRoutes(Ra,Rr), D='R', deviationAt(Ra,Rr,P), writeln(P).
hasNoRouteDeviation(V,_) :- hasAssignedRoute(V,Ra), hasActualRoute(V,Rr), areEqualRoutes(Ra,Rr).

areEqualRoutes([],[]).
areEqualRoutes([(X,Y)|RestCoordsA],[(X,Y)|RestCoordsB]) :- areEqualRoutes(RestCoordsA,RestCoordsB).

areNotEqualRoutes([(_,_)|_],[]).             % cardinality of first coordinate set is larger
areNotEqualRoutes([],[(_,_)|_]).             % cardinality of second coordinate set is larger

% if coordinates are same, check others
areNotEqualRoutes([],[]):- fail.
areNotEqualRoutes([(X,Y)|RestCoordsA],[(X,Y)|RestCoordsB]) :- !,areNotEqualRoutes(RestCoordsA,RestCoordsB).
areNotEqualRoutes([(X,Y)|_],[(X,Y1)|_]).     % if x is different
areNotEqualRoutes([(X,Y)|_],[(X1,Y)|_]).     % if y is different

deviationAt([(X,Y)|_],[],(X,Y)).             % cardinality of first coordinate set is larger
deviationAt([],[(X,Y)|_],(X,Y)).             % cardinality of second coordinate set is larger

deviationAt([],[],_):- fail.
deviationAt([(X,Y)|RestCoordsA],[(X,Y)|RestCoordsB],D) :- !,deviationAt(RestCoordsA,RestCoordsB,D).
deviationAt([(X,Y)|_],[(X,Y1)|_],(X,Y)).     % if x is different
deviationAt([(X,Y)|_],[(X1,Y)|_],(X,Y)).     % if y is different
...
hasNoJustification(V,A) :- (noJamJustify(A); not(trafficJam(V,_,_))),
                           (accidentJustify(A); not(trafficAccident(V,_,_))),
                           (weatherJustify(A); not(weatherDelay(V,_,_))).

jamJustify(X) :- X = 'T'; X = 'R'.
noJamJustify(X) :- X \= 'T', X \= 'R'.

accidentJustify(X) :- X = 'T'; X = 'R'.
noAccidentJustify(X) :- X \= 'T', X \= 'R'.

weatherJustify(X) :- X = 'T'; X = 'R'.
noWeatherJustify(X) :- X \= 'T', X \= 'R'.
%-------------------------------------------------------------

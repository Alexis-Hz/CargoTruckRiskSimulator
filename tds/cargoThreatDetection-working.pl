...
isNotMonitored(V) :- not(monitor(V)).

hasAssignedRoute(V,R) :- assignedRoute(V,R).
hasNoAssignedRoute(V,R) :- not(assignedRoute(V,R)).

hasActualRoute(V,R) :- actualRoute(V,R).
hasNoActualRoute(V,R) :- not(actualRoute(V,R)).

hasExpectedArrivalTime(V,T) :- expectedArrivalTime(V,T).
hasNoExpectedArrivalTime(V,T) :- not(expectedArrivalTime(V,T)).

hasActualArrivalTime(V, T) :- actualArrivalTime(V,T).
hasNoActualArrivalTime(V, T) :- not(actualArrivalTime(V,T)).
...
% --- rule to determine truth of red light
%hasThreat(V) :- hasAnomaly(V,A), not(hasJustification(V,A)).
hasThreat(V) :- hasAnomaly(V,[],_), hasAnomalyList(V,List), hasNoJustification(V,List).

% --- rule to determine truth of yellow light
hasJustifiedThreat(V) :- hasAnomaly(V,[],_), hasAnomalyList(V,List), hasJustification(V,List).

% --- rulte to determine truth of green light
hasNoThreat(V) :- hasNoAnomaly(V,A).

% --- hasAnomaly,hasNoAnomaly
hasAnomaly(V,EmptyList,AnomalyList) :-
                   hasMonitoringAnomaly(V,EmptyList,AnomalyList1);
                   hasTimeDeviation(V,AnomalyList1,AnomalyList2);
                   hasRouteDeviation(V,AnomalyList2,AnomalyList3);
                   hasAlarm(V,AnomalyList3,AnomalyList4);
                   hasPatternViolation(V,AnomalyList4,AnomalyList).
                   
hasNoAnomaly(V,A) :- hasNoMonitoringAnomaly(V,A),
                     hasNoTimeDeviation(V,A),
                     hasNoRouteDeviation(V,A),
                     hasNoAlarm(V,A).
% FIX THIS LATER     hasNoPatternViolation(V,A).

% --- anomaly_list,anomaly_list1

hasAnomalyList(V,List) :- anomaly_list1(V,[],List).

anomaly_list1(V,EmptyList,AnomalyList) :-
                   hasMonitoringAnomaly(V,EmptyList,AnomalyList);
                   hasTimeDeviation(V,EmptyList,AnomalyList);
                   hasRouteDeviation(V,EmptyList,AnomalyList);
                   hasAlarm(V,EmptyList,AnomalyList);
                   hasPatternViolation(V,EmptyList,AnomalyList).
                   

% --- hasMonitoringAnomaly,hasNoMonitoringAnomaly
hasMonitoringAnomaly(V,A,B) :- isNotMonitored(V), merge_set(A,['M'],B);
                             hasNoAssignedRoute(V,_), merge_set(A,['M'],B);
                             hasNoActualRoute(V,_), merge_set(A,['M'],B);
                             hasNoExpectedArrivalTime(V,_), merge_set(A,['M'],B);
                             hasNoActualArrivalTime(V,_), merge_set(A,['M'],B).


hasNoMonitoringAnomaly(V,_) :- isMonitored(V),
                               hasAssignedRoute(V,_),
                               hasActualRoute(V,_),
                               hasExpectedArrivalTime(V,_),
                               hasActualArrivalTime(V,_).
                             
% --- hasTimeDeviation,hasNoTimeDeviation
hasTimeDeviation(V,D,B) :- hasExpectedArrivalTime(V,Te), hasActualArrivalTime(V,Ta), notOnTime(Te,Ta),merge_set(D,['T'],B).
hasNoTimeDeviation(V,_) :- hasExpectedArrivalTime(V,Te), hasActualArrivalTime(V,Ta), onTime(Te,Ta).

timeTolerance(T) :- T = 5.
onTime(Te,Ta) :- timeTolerance(E), Ta > Te - E, Ta < Te + E.
notOnTime(Te,Ta) :- timeTolerance(E), Ta < Te - E; timeTolerance(E), Ta > Te + E.

% --- hasRouteDeviation,hasNoRouteDeviation
hasRouteDeviation(V,D,B) :- hasAssignedRoute(V,Ra), hasActualRoute(V,Rr), not(Ra = Rr), merge_set(D,['R'],B).
hasNoRouteDeviation(V,_) :- hasAssignedRoute(V,Ra), hasActualRoute(V,Rr), Ra = Rr.
...
% --- hasAlarm,hasNoAlarm
hasAlarm(V, A, B) :- alarmOn(V,_,_), merge_set(A,['A'],B).
hasNoAlarm(V,_) :- not(alarmOn(V,_,_)).
...
% --- hasPatternViolation,hasNoPatternViolation
hasPatternViolation(V,A,B) :- usualPort(V, Pu), thisPort(V, P), Pu \= P, merge_set(A,['PP'],B);
                            usualShipper(V, Su), thisShipper(V, S), Su \= S, merge_set(A,['SP'],B);
                            usualBroker(V, Bu), thisBroker(V, B), Bu \= B, merge_set(A,['BP'],B);
                            usualCargo(V, Cu), thisCargo(V, C), Cu \= C, merge_set(A,['CP'],B);
                            usualHours(V, Hu), thisHours(V, H), Hu \= H, merge_set(A,['HP'],B);
                            usualDriver(V, Du), thisDriver(V, D), Du \= D, merge_set(A,['DP'],B);
                            usualOrigin(V, Ou), thisOrigin(V, O), Ou \= O, merge_set(A,['OP'],B);
                            usualDestination(V, Dtu), thisDestination(V, Dt), Dtu \= Dt, merge_set(A,['FP'],B);
                            usualTransportCo(V, Tcu), thisTransportCo(V, Tc), Tcu \= Tc, merge_set(A,['PP'],B).

hasNoPatternViolation(V,_) :- usualPort(V, Pu), thisPort(V, P), Pu = P,
                              usualShipper(V, Su), thisShipper(V, S), Su = S,
                              usualBroker(V, Bu), thisBroker(V, B), Bu = B,
                              usualCargo(V, Cu), thisCargo(V, C), Cu = C,
                              usualHours(V, Hu), thisHours(V, H), Hu = H,
                              usualDriver(V, Du), thisDriver(V, D), Du = D,
                              usualOrigin(V, Ou), thisOrigin(V, O), Ou = O,
                              usualDestination(V, Dtu), thisDestination(V, Dt), Dtu = Dt,
                              usualTransportCo(V, Tcu), thisTransportCo(V, Tc), Tcu = Tc.

% --- hasJustification/hasNoJustification
...
hasNoJustification(V,A) :- (noJamJustify(A); not(trafficJam(V,_,_))),
                           (noAccidentJustify(A); not(trafficAccident(V,_,_))),
                           (noWeatherJustify(A); not(weatherDelay(V,_,_))).

jamJustify(X) :- member('T',X);member('R',X).
noJamJustify(X) :- not(member('T',X)),not(member('R',X)).

accidentJustify(X) :- member('T',X);member('R',X).
noAccidentJustify(X) :- not(member('T',X)),not(member('R',X)).

weatherJustify(X) :- member('T',X);member('R',X).
noWeatherJustify(X) :- not(member('T',X)),not(member('R',X)).
%-------------------------------------------------------------
